...
public class CustomReporter implements IReporter {
	...
	private static SimpleDateFormat sdftime = new SimpleDateFormat("hh:mm:ss.SSS a");
	public static String outFilename = "Agoda-Automation-Report.html";
	...
	protected void writeHead() {
		writer.print("<head>");
		writer.print("<title>API Automation Report</title>");
		writeStylesheet();
		writer.print("</head>");
	}
	...
	protected void writeStylesheet() {
		...
		writer.print(".passedodd td {background-color: #BFB}");// 3F3
		writer.print(".passedeven td {background-color: #BFB}");// 0A0
		writer.print(".skippedodd td {background-color: #DDD}");// DDD
		writer.print(".skippedeven td {background-color: #CCC}");// CCC
		writer.print(".failedodd td,.attn {background-color: #FAA}");// F33
		writer.print(".failedeven td,.stripe .attn {background-color: #FAA}");// D00
	...
	protected void writeBody() {
		writer.print("<body>");
		writeReportTitle("API Automation Test Report");
		writeSuiteSummary();
	...
	protected void writeSuiteSummary() {
		...
		writer.print("<th>Total Duration (Seconds)</th>");
		writer.print("<th>Included Groups</th>");
		writer.print("<th>Excluded Groups</th>");
		writer.print("<th>Wallet Server Host</th>");
		writer.print("</tr>");
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			// String[] suites = GlobalConstants.File_TestSuites.split(",");
			for (TestResult testResult : suiteResult.getTestResults()) {
				int passedTests = testResult.getPassedTestCount();
				// GlobalConstants.passcount = passedTests;
				int skippedTests = testResult.getSkippedTestCount();
				// GlobalConstants.skipcount = skippedTests;
				int failedTests = testResult.getFailedTestCount();
				// GlobalConstants.failcount = failedTests;
				long duration = testResult.getDuration();
				// GlobalConstants.totalexecutiontime = duration;
				writer.print("<tr");
				if ((testIndex % 2) == 1) {
		...
		if (testIndex >= 1) {
			...
			writeTableHeader(integerFormat.format(totalPassedTests) + " ("
					+ decimalFormat.format(((double) totalPassedTests
							/ (double) (totalPassedTests + totalSkippedTests + totalFailedTests)) * (double) 100)
					+ "%)", "num");
	...
	protected void writeScenarioSummary() {
		...
		writer.print("<thead>");
		writer.print("<tr>");
		writer.print("<th>Test Class Name</th>");
		writer.print("<th>Test Method Name</th>");
		writer.print("<th>Test Description</th>");
		writer.print("<th>Start Time</th>");
		writer.print("<th>End Time</th>");
		writer.print("<th>Duration (Seconds)</th>");
		writer.print("</tr>");
		writer.print("</thead>");
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					String[] descriptions = aResult.getMethod().getDescription().split(":");
					...
					buffer.append("<td><a href=\"#m").append(scenarioIndex).append("\">")
							.append(methodName + "</a></td>").append("<td rowspan=\"").append(1).append("\">")
							.append(descriptions[0]).append("</td>").append("<td rowspan=\"").append(1).append("\">")
							.append(parseUnixTimeToTimeOfDay(start)).append("</td>").append("<td rowspan=\"").append(1)
							.append("\">").append(parseUnixTimeToTimeOfDay(end)).append("</td>")
							.append("<td rowspan=\"").append(1).append("\">")
					...
					for (int i = 1; i < resultsCount; i++) {
						// ===============================================================
						ITestResult aResult1 = results.get(i);
						String methodName1 = Utils.escapeHtml(aResult1.getMethod().getMethodName());
						long start1 = aResult1.getStartMillis();
						long end1 = aResult1.getEndMillis();
						long duration1 = aResult1.getEndMillis() - start1;
						// ===============================================================

						buffer.append("<tr class=\"").append(cssClass).append("\">").append("<td><a href=\"#m")
								.append(scenarioIndex).append("\">").append(methodName1 + "</a></td>")
								.append("<td rowspan=\"1\">" + descriptions[i] + "</td>").append("<td rowspan=\"")
								.append(1).append("\">").append(parseUnixTimeToTimeOfDay(start1)).append("</td>")
								.append("<td rowspan=\"").append(1).append("\">").append(parseUnixTimeToTimeOfDay(end1))
								.append("</td>").append("<td rowspan=\"").append(1).append("\">")
								.append(decimalFormat.format(millisecondsToSeconds(duration1))).append("</td></tr>");
						scenarioIndex++;
					}
	...
	protected static class TestResult {
		...
		protected static final Comparator<ITestResult> RESULT_COMPARATOR = new Comparator<ITestResult>() {
			...
			public int compare(ITestResult o1, ITestResult o2) {
				...
				String o1f = formatter.format(o1.getMethod().getPriority());
				String o2f = formatter.format(o2.getMethod().getPriority());
				// int result = 0;//
				// o1.getTestClass().getName().compareTo(o2.getTestClass().getName());
				// if (result == 0) {
				int result = (o1f + o1.getMethod().getMethodName()).compareTo(o2f + o2.getMethod().getMethodName());
				// }
				return result;
			}
		};
	...
	@SuppressWarnings("unused")
	private String getSuiteNameForReport(String suite) {
		String postfix = "APITest_TestSuite";
		try {
			postfix = suite.substring(suite.indexOf("_"), suite.indexOf("."));
		} catch (Exception e) {
		}
		return "APITest" + postfix;
	}
}
